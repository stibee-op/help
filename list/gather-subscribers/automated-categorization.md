...
구독자를 특정 그룹으로 자동 분류하거나, 사용자 정의 필드에 원하는 값을 자동으로 추가하고 싶다면 '파라미터' 기능을 활용하면 됩니다. 이 방법에 관해 알아봅니다.
...
예를 들어, 구독 폼 URL에 이름(name) 필드를 파라미터로 미리 입력했을 때와 입력하지 않았을 때를 비교해 보면,

* 파라미터가 적용되지 않은 구독 폼 URL: https://page.stibee.com/subscriptions/187957
  * 이름 필드가 비어 있습니다.
...
* https://page.stibee.com/subscriptions/3?groupIds=44737 (페이스북 그룹)
* https://page.stibee.com/subscriptions/3?groupIds=43677 (인스타그램 그룹)
...
그룹과 관련한 자세한 내용은 [그룹 사용하기](../classify-subscribers/how-to-use-groups.md) 도움말을 참고해 주세요.
{% endhint %}
...
* 특정 그룹에 구독자를 자동 분류하는 구독 폼: https://page.stibee.com/subscriptions/{listid}?groupIds={groupid}
...
예를 들어, 주소록 listid가 3이고, 그룹 ID가 44737이라면 URL은 다음과 같습니다.
...
구독 폼 HTML 코드 뒤에 ?groupIds={groupid} 값을 추가하면 됩니다. 예를 들어, 그룹 아이디가 12345인 경우 아래와 같이 url을 수정합니다.
...
사용자 정의 필드 관련한 자세한 내용은 [구독자 정보 이해하기](../adding-managing-subscriber/understanding-subscriber-info.md) 도움말을 참고해 주세요.
{% endhint %}
...
예를 들어, 사용자 정의 필드 '유입 경로'의 키(key)가 route이고, 값에 stibee를 자동 입력하고 싶다면 다음과 같이 입력해 주세요.
...
구독 폼 HTML 코드 뒤에 ?field\_key={field\_value} 값을 추가하면 됩니다. 예를 들어, 가령, 키(key) 값이 channel인 필드에 '랜딩 페이지' 값을 자동으로 입력하고 싶다면 아래와 같이 입력합니다.
