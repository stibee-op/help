...
이미 주소록에 등록된 구독자 정보를 추가하는 경우, 데이터를 어떻게 처리할지 여부를 결정할 수 있는 옵션입니다. 이 옵션을 체크하면 나중에 추가되는 구독자 정보의 특정 필드 값(예: 이름)이 비어있는 경우 기존에 저장된 값은 사라지고 빈 값으로 업데이트됩니다. _구독자의 정보가 중복되는 정보인지 여부는 이메일 주소를 기준으로 판단합니다._

예를 들어, dooly@stibee.com 구독자의 이름을 '둘리'로 입력하고, \[추가하는 필드에 값이 없으면 기존 값을 삭제하고 빈 값으로 업데이트합니다.] 옵션을 체크한 뒤 구독자의 이름을 비어 있는 상태로 추가하면, 이메일 주소는 그대로 남지만 이름은 빈 값으로 업데이트됩니다.
