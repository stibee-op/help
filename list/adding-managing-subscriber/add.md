...
파일로 구독자 정보를 추가할 때 이메일 주소 외에 이름, 핸드폰 번호 등 다른 정보가 있다면 파일에 있는 어떤 열이 각각 '이름' 또는 '핸드폰 번호'의 정보에 해당하는지 선택해야 합니다. 만약 파일에는 다른 정보가 있지만 이 단계(아래 사진)에서 선택해야 하는 필드가 없다면 [사용자 정의 필드](understanding-subscriber-info.md#undefined)에서 추가하고 싶은 정보를 추가한 뒤에 다시 시도해주세요.
