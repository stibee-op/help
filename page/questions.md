...
<summary>페이지에 발행한 이메일을 수정할 수 있나요?</summary>

이메일 제목과 설명은 발송 완료된 이메일에서도 바로 수정할 수 있습니다. 이메일 목록에서 \[더 보기(∨) → 발행 설정하기]를 클릭하고 제목과 설명을 수정한 뒤 \[저장하기]를 누르면 페이지와 아카이브에 반영됩니다.&#x20;

다만, 페이지와 아카이브에 발행된 본문을 수정하는 기능은 아직 제공하지 않습니다. 아래 방법으로 수정한 이메일을 다시 발행할 수 있습니다.
...
5. 발송이 완료된 이메일을 다시 발행합니다. 이전에 발행한 이메일은 \[발행 취소하기]를 진행해 주세요.

_\* 이때, 발송 일자는 달라지는 점 참고해 주세요._

{% hint style="warning" %}
구독자의 메일함으로 전달된 이메일은 수정할 수 없습니다. 이미 전달된 이메일은 구독자가 사용하는 이메일 서비스(예: G메일, 네이버 메일 등)에서만 관여할 수 있기 때문입니다.
{% endhint %}
...
페이지는 기능이 확장된 아카이브(아카이빙 페이지) 입니다.
...
요금제에 대한 자세한 설명은 [요금제와 결제](https://app.gitbook.com/s/eAMHRdY4ATDXfWZWQs3p/pricing)를 참고해 주세요.
...
\[페이지 → 구글 애널리틱스 연동 → 연동하기]를 눌러 구글 애널리틱스와 페이지를 연동할 수 있어요. 자세한 내용은 [여기](creating-and-managing/analytics.md)를 참고해 주세요.
