...
이메일이 발송 실패되는 경우를 '반송, 바운스(Bounce)'라고 합니다. 바운스는 발송 실패 원인에 따라 '소프트바운스(Softbounce)'와 '하드바운스(Hardbounce)'로 나뉩니다.&#x20;
...
주소록에 있는 모든 구독자의 통계 데이터를 파일로 내려받을 수 있는 기능입니다. \[주소록 → 구독자 목록 → 내보내기 → 구독자별 통계 내보내기]를 눌러 진행할 수 있어요.
