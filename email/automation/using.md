...
\[자동 이메일]은 구독자 한 명, 한 명에게 정확히 보내는 구조에 최적화되어 있습니다. 따라서, 자동 이메일을 한 번에 여러 명의 구독자에게 보내는 경우, [일반 이메일](https://app.gitbook.com/s/eAMHRdY4ATDXfWZWQs3p/email)과 처리 방식이 달라 발송 완료까지 시간이 오래 걸릴 수 있습니다. 스티비의 자동 이메일 기능을 사용하는 구체적인 방법에 대해 알아봅니다.
...
3. \[자동 이메일]이 만들어지고 이메일이 발송될 \[[주소록](https://app.gitbook.com/s/eAMHRdY4ATDXfWZWQs3p/list)]을 선택하는 단계로 이동합니다.
...
자동 이메일 트리거는 기본적으로 중복 요청을 허용하지 않습니다. 중복 발송 요청을 허용하면 특정 구독자가 조건에 맞을 때마다 트리거가 작동해 자동 이메일이 과도하게 발송될 수 있기 때문입니다.

그러나 트리거의 중복 요청을 허용해야 하는 경우도 있습니다. 예를 들어, 스티비를 사용해 '회원가입 인증 이메일'을 발송하는 경우, 회원이 인증 메일을 여러 번 요청할 수 있습니다. 이때 트리거 중복 요청을 허용하지 않는다면 두 번째 요청부터는 인증 메일이 발송되지 않습니다.

아래 방법으로 \[트리거 중복 허용하기]를 진행할 수 있습니다.

1. 자동 이메일의 \[발송 조건] 설정 단계로 이동합니다.
2. 트리거 설정 아래쪽에 있는 \[트리거 중복 허용하기]를 활성화합니다. 다시 비활성화하고 싶을 때는 \[트리거 중복 허용하기]를 한 번 더 클릭합니다.

**\*주의:** 아래 트리거는 트리거의 동작 방식 자체가 중복 요청을 처리하기 어려워, \[트리거 중복 허용하기]를 사용할 수 없습니다.

* '기념일' 트리거: \[트리거 중복 허용하기] 옵션이 노출되지 않습니다.
* '오픈 안 함', '링크 클릭 안 함' 트리거: \[트리거 중복 허용하기] 옵션은 노출되지만, 활성화하더라도 중복 요청은 처리되지 않습니다.



**중복 제한 시간 설정하기**

\[트리거 중복 허용하기]를 활성화했다면, 중복 요청을 허용하는 시간을 추가로 제한할 수 있습니다. 동일한 구독자가 설정한 시간 안에 트리거 조건을 다시 충족하더라도 이메일을 발송하지 않습니다.

예를 들어, \[오픈] 트리거를 사용하며 중복 요청 제한 시간을 1시간으로 설정한 경우,

* A 구독자가 조건이 되는 이메일을 오전 10:00에 오픈하면, 트리거 조건을 충족했으므로 이메일이 발송됩니다.
* 이후 A 구독자가 같은 이메일을 오전 10:30에 다시 오픈하면, 중복 제한 시간 설정으로 인해 이메일이 발송되지 않습니다.
* A 구독자가 같은 이메일을 오전 11:10에 다시 오픈하면, 중복 제한 시간이 이미 지났기 때문에 이메일이 다시 발송됩니다.

_\* 중복 요청 시간 제한을 두지 않으려면, 제한 시간을 0분으로 설정합니다._
...
예를 들어, '월요일 오전 9:00부터 수요일 오후 17:00까지' 이메일이 발송되도록 설정했다면,
...
예를 들어, A 자동 이메일 발송 시간대를 '매주 월요일에서 금요일, 매일 오전 10:00부터 오후 17:00까지' 발송되도록 설정했다면,

* A 자동 이메일은 월요일에서 금요일, 오전 10:00부터 오후 17:00까지만 발송됩니다.
...
B 자동 이메일의 발송 시간대를 '매일 오후 15:00부터 오전 9:00까지'로 설정했다면,
...
* 즉, 오전 9:01부터 오후 14:59까지는 트리거 조건을 만족해도 B 이메일은 발송되지 않으며, 다음날 오전 9:00가 지난 시점에는 트리거 조건을 만족해도 B 이메일은 발송되지 않습니다.

**사례 2. 발송 시간대의 시작 시간이 끝 시간보다 크고, 요일이 제한되어 있는 경우**

C 이메일 발송 시간대를 '월요일 오후 3:00부터 화요일 오전 9:00까지'로 설정했다면,
...
발송 조건을 설정하는 것 이외의 이메일 편집 단계는 일반 이메일과 같습니다. 각 단계 제목 앞에 \[V] 표시가 됐다면 모든 단계의 입력을 완료한 것이고, 이제 자동 이메일 발송을 시작할 수 있습니다.
