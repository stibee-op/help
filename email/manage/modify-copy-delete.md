...
작성중 상태인 이메일은 언제든지 목록에서 이메일의 제목을 클릭해 수정할 수 있습니다. 이메일의 제목을 클릭해도 되고, 목록 오른쪽에 있는 \[더 보기(∨) → 수정하기]를 클릭해 수정 화면으로 이동할 수도 있습니다.

{% hint style="warning" %}
이미 발송 완료된 이메일은 수정할 수 없습니다. 이미 전달된 이메일은 구독자가 사용하는 이메일 서비스(예: G메일, 네이버 메일 등)에서만 다룰 수 있기 때문입니다. 다만, 페이지, 아카이브에 표시되는 제목과 설명은 수정할 수 있습니다. 자세한 방법은 [이메일 아카이빙하기](../share/archive.md)를 참고해 주세요.
{% endhint %}
...
\[더 보기(∨) → 삭제하기] 를 클릭하면 확인 단계를 거쳐 이메일을 삭제합니다. 삭제한 이메일은 DB에서도 즉시 삭제되기 때문에 복구할 수 없습니다.
