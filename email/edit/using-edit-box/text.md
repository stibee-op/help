...
### 텍스트 스타일 설정하기 <a href="#style" id="style"></a>

폰트 종류, 크기, 정렬 방식 등 텍스트 스타일을 설정할 수 있습니다.

* 폰트: 스티비 에디터에서는 고딕 계열, 명조 계열 두 가지를 제공합니다.
  * 이메일 환경에서는 웹 폰트 사용이 불가하여, 두 가지 계열의폰트를 제공합니다.
...
대부분의 이메일 수신 환경(예. G메일, 네이버 메일 등)은 웹폰트 등 특수한 폰트를 지원하지 않습니다. 따라서, 스티비에서는 '고딕 계열, 명조 계열' 폰트만 사용할 수 있도록 제공하고 있습니다.&#x20;

예를 들어, 스티비 에디터에서 폰트를 '고딕 계열'로 설정한 경우, 구독자의 수신 환경에 설치된 고딕 계열의 폰트로 표시됩니다.
...
* 작은따옴표, 큰따옴표, 쉼표가 포함된 URL은 정상적으로 처리되지 않습니다.
...
* 단축 URL 사용 시 스팸 이메일로 분류될 확률이 높아집니다.
  * 링크 단축 서비스를 사용해서 URL을 단축하는 경우, 해당 URL이 포함된 이메일은 스팸으로 분류될 확률이 높아집니다. 되도록 원본 URL을 사용하는 것을 권장합니다.

### 텍스트에 메일머지 사용하기 <a href="#mail-merge" id="mail-merge"></a>
