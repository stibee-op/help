...
* 폰트: 스티비 에디터에서는 고딕, 명조 두 가지를 제공합니다.
  * 이메일 환경에서는 웹 폰트를 사용이 불가하여, 두 가지 폰트를 제공합니다.
...
예를 들어, 스티비 에디터에서 폰트들 '고딕'으로 설정한 경우, 구독자의 수신 환경에 설치된 고딕 계열의 폰트로 표시됩니다.
...
**\*주의:** URL 사용 시 아래 사항을 주의해 주세요.

* 작은따옴표, 큰따옴표, 쉼표가 포함된 URL은 정상적으로 처리되지 않는다.
  * 작은따옴표('), 큰따옴표("), 쉼표(,)가 포함된 URL을 스티비를 통해 발송하면 링크가 정상적으로 처리되지 않으며, 링크 접속이 불가합니다. 이러한 특수문자가 포함된 URL을 사용해야 하는 경우, URL 인코딩을 진행한 뒤 사용해 주세요.
  * URL 인코딩은 [여기](https://www.urlencoder.org/)에서 진행할 수 있습니다.
* 단축 URL 사용 시 스팸 이메일로 분류될 확률이 높아진다.
