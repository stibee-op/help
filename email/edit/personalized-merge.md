...
이 상황에 gildong.go@stibee.com라는 구독자를 등록하고 이 구독자의 페이지 필드 값에 stibee.com을 등록하고 아래와 같이 설정해서 발송하게 되면 사용자가 설정한 필드의 키 값이 기본 메일머지보다 우선으로 적용되기 때문에 아래와 같이 변환되어 발송됩니다. 따라서 \[사용자 정의 필드]의 키값을 설정할 때는 기본 필드의 키값과 중복되지는 않는지 여부를 확인해보는 것이 좋습니다.
